...
class OrdersController {
  ...
  static async addProductToOrder(req, res, next) {
    const { oid } = req.params;
    const payload = req.body;
    try {
      const addedProductToOrder = await ordersService.addProductToOrder(
        oid,
        payload
      );
      const response = {
  ...
  static async deleteProductToOrder(req, res, next) {
    const { oid } = req.params;
    const payload = req.body;
    try {
      const deletedProductToOrder = await ordersService.deleteProductToOrder(
        oid,
        payload
      );
      const response = {
        success: true,
        deletedProductToOrder,
      };
      res.status(HTTP_STATUS.OK).json(response);
    } catch (error) {
      next(error);
  ...
  static async deleteOrders(req, res, next) {
    try {
      const deletedOrders = await ordersService.deleteOrders();
      const response = {
        success: true,
        deletedOrders,
      };
