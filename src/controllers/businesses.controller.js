...
class BusinessesController {
  ...
  static async deleteProductToBusiness(req, res, next) {
    const { bid } = req.params;
    const payload = req.body;
    try {
      const deletedProductToBusiness =
        await businessesService.deleteProductToBusiness(bid, payload);
      const response = {
        success: true,
        deletedProductToBusiness,
      };
      res.status(HTTP_STATUS.OK).json(response);
    } catch (error) {
      next(error);
  ...
  static async deleteBusinesses(req, res, next) {
    try {
      const deletedBusinesses = await businessesService.deleteBusinesses();
      const response = {
        success: true,
        deletedBusinesses,
      };
