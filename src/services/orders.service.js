...
class OrdersService {
  ...
  async addProductToOrder(orderId, product) {
    ...
    const orderDB = await ordersDAO.getOrderById(orderId);
    if (!orderDB) {
      throw new HttpError("Order not found", HTTP_STATUS.NOT_FOUND);
    }

    const response = await ordersDAO.addProductToOrder(orderId, product);
    return response;
  }

  async deleteProductToOrder(orderId, product) {
    if (!orderId || !product) {
      throw new HttpError("Missing param", HTTP_STATUS.BAD_REQUEST);
    }
    ...
    const response = await ordersDAO.deleteProductToOrder(orderId, product);
    return response;
  }
  ...
  async deleteOrders() {
    const deletedOrders = await ordersDAO.deleteOrders();
