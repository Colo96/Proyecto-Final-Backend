...
class BusinessesService {
  ...
  async deleteProductToBusiness(businessId, product) {
    if (!businessId || !product) {
      throw new HttpError("Missing param", HTTP_STATUS.BAD_REQUEST);
    }
    ...
    const response = await businessesDAO.deleteProductToBusiness(
      businessId,
      product
    );
    return response;
  ...
  async deleteBusinesses() {
    const deletedBusinesses = await businessesDAO.deleteBusinesses();
