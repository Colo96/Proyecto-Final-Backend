...
class BusinessesDAO {
  ...
  async deleteProductToBusiness(id, product) {
    const deletedProductToBusiness = await BusinessesModel.updateOne(
      { _id: id },
      { $pull: { products: product._id } }
    );
    return deletedProductToBusiness;
  }

  async deleteBusinesses() {
    const deletedBusinesses = await BusinessesModel.deleteMany();
